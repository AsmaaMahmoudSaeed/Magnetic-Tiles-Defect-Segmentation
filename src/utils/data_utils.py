...
def dataset_perp(dataset_url,
                 data_dir,
                 orig_dirname,
                 new_dirname):
    dataset_path=data_dir/new_dirname
    # Removes the existing dataset directory if it already exists to ensure a clean setup.
    ...
    download_and_unzip(url=dataset_url,
     ...
     #Creates images/ and masks/ subdirectories in the dataset directory to store images and masks. 
    ...
    # Retrieves file paths for images (.jpg) and masks (.png) from specific defect-type subdirectories.
    ...
    for img_fpath,msk_fpath in tqdm(zip(img_fpaths,msk_fpaths),total=len(img_fpaths)):
        msk=np.array(Image.open(msk_fpaths))
        msk[msk>0]=1
        ...
        # Converts the binarized mask back to a PIL image and saves it to the masks/ directory.
    ...
    def creat_test_dataset(img_dir_path,
                        mask_dir_path,
                        train_img_dir_path,
                        train_mask_dir_path,
                        test_img_dir_path,
