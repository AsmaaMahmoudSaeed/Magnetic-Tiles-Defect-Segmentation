...
def train_model(train_img_dir_path,
    ...
    dls=SegmentationDataLoaders.from_label_func(num_workers=0,
                                path=train_img_dir_path.parent,
                                fnames=fpaths,
                                label_func=partial(get_mask_path,train_mask_dir_path=train_mask_dir_path),
                                codes=[0,1],
                                bs=batch_size,
    ...
    # for studio.datachian
    learn.fine_tune(n_epochs,cbs=[DVCLiveCallback(dir="training",report=None)])
    learn.export(fname=Path(model_pickl_fpath).absolute())